...
fun isCoordinateInsideChess(coord: Pair<Int, Int>, numColumns: Int, numLines: Int): Boolean {
    return when {
        (coord!=null && coord.first in 1..numLines && coord.second in 1..numColumns) -> true
        else -> false
    }
}
            ...
            if (jogada1 == "m" || jogada1 == "M") {return}
                ...
                if (jogada1.length == 2) {
                    ...
                    if ((isCoordinateInsideChess(getCoordinates(jogada1)!!, numColumns, numLines)
                        ...
                        if (isCoordinateInsideChess(getCoordinates(jogada1)!!, numColumns, numLines)
                                && checkRightPieceSelected(cor, totalPiecesAndTurn[2])) {println(player1Target)
                            jogada1Target = readLine().toString()
                            if (jogada1Target == "m" || jogada1Target == "M") {return}
                            val gcT = getCoordinates(jogada1Target)
                            if (gcT == null) {println(respostaInvalida)}
                                ...
                                if (!movePiece(pieces, numColumns, numLines, getCoordinates(jogada1)!!,getCoordinates(jogada1Target)!!,
                                                totalPiecesAndTurn)) {println(respostaInvalida)}
            ...
            if (jogada2 == "m" || jogada2 == "M") {return}
                ...
                if (jogada2.length == 2) {
                    ...
                    if (isCoordinateInsideChess(getCoordinates(jogada2)!!, numColumns, numLines)
                        ...
                        if (isCoordinateInsideChess(getCoordinates(jogada2)!!, numColumns, numLines)
                                && checkRightPieceSelected(cor, totalPiecesAndTurn[2])) {println(player2Target)
    ...
    while (menu != 2) {
        ...
        when (menu) {
            1 -> {
                ...
                if(!totalPiecesAndTurn.isEmpty() && !pieces.isEmpty()) {
